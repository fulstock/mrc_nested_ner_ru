...
jsonpath = "data/RuNNE_structallinn/test.json" # Здесь выбираем, куда будет сохраняться датасет, и под каким именем
...
for tag, entities in tag_to_spans.items():
    ...
    for context in all_contexts:
        ...
        innermost_entities = [v for v in context_entities if \
            not any([(s[1], s[2]) for s in context_entities if v is not s and v[1] <= s[1] and v[2] >= s[2]])]
        innermost_entities = sorted(innermost_entities, key = lambda x: x[1])

        # if len(innermost_entities) != len(context_entities):
           # print(context)
           # print([(v[1], v[2]) for v in context_entities])
           # print([(v[1], v[2]) for v in innermost_entities])
        ...
        # print(context)
        # print([(v[1], v[2]) for v in innermost_entities])
        for entity in innermost_entities:
            span = entity[0]["span"]
