...
dataset_name = "NEREL-bio-v1.0_con"
...
for ds in sets:
    ...
    for tag, entities in tag_to_spans.items():
        ...
        for entity in entities:
            ...
            for span in sentence_spans:
                ...
                if entity["span"] in context and entity["start"] >= start and entity["end"] <= end:
                    ent_cont.append((context, entity))
        tag_to_spans[tag] = ent_cont
        span_count = [v[1]["span"] for v in ent_cont]
        span_count = Counter(span_count)
        tag_to_spans[tag] = [(v[0], v[1], span_count[v[1]["span"]]) for v in tag_to_spans[tag]]
        ...
        if tag == "DISO":
            print(*[t[0] for t in tag_to_spans[tag]], sep = "\n\n")

        context = tag_to_spans[tag][0][0]
        full_query = context
        # for n in range(10):
        #     full_query += " [SEP] " + span_set[n][0] + " - это " + tag + "."
        tag_to_spans[tag] = full_query
